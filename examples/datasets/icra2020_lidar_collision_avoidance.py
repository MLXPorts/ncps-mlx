...
def _unpack(npz_path: str) -> list[tuple[mx.array, mx.array]]:
    sequences: list[tuple[mx.array, mx.array]] = []
    with mx.load(npz_path) as packed:
        for idx in range(29):
            sequences.append((packed[f"x_{idx}"], packed[f"y_{idx}"]))
    return sequences


def _train_test_split(files: list[tuple[mx.array, mx.array]]):
    rng = mx.random.seed(20200822)
    rng.shuffle(files)
...
def _align_in_sequences(data, seq_len: int):
    ...
    xs_arr = mx.stack(xs, axis=0)
    ys_arr = mx.stack(ys, axis=0)
    xs_arr = mx.expand_dims(xs_arr, axis=-1)
    ys_arr = mx.expand_dims(ys_arr, axis=-1)
