#!/usr/bin/env python
"""
NCPSLint: A linting tool for ncps-mlx codebase.
...
2. MLX purity issues:
   ...
   - Direct Python operators instead of MLX operations
3. Style issues (PEP 8)
4. Import issues (unused, missing)
5. Type annotation issues

It helps ensure that ncps-mlx code uses MLX properly and remains maintainable.
"""
...
def check_backend_specific_imports(file_path: str) -> Tuple[bool, List[str]]:
    ...
    DISABLED for ncps-mlx: This is an MLX-specific project, so MLX imports are expected.
    Only warn about PyTorch/TensorFlow imports which shouldn't be here.
    """
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Only check for PyTorch and TensorFlow (which shouldn't be in MLX code)
    bad_imports = []
    
    patterns = [
        r'import\s+torch\b',  # import torch
        r'from\s+torch\s+import\s+(.*)',  # from torch import ...
        r'import\s+tensorflow\b',  # import tensorflow
        r'from\s+tensorflow\s+import\s+(.*)',  # from tensorflow import ...
    ]
    ...
    for pattern in patterns:
        ...
        if matches:
            if 'torch' in pattern:
                bad_imports.append("torch (should use MLX)")
            elif 'tensorflow' in pattern:
                bad_imports.append("tensorflow (should use MLX)")
    
    return bool(bad_imports), bad_imports
...
def check_backend_specific_code(file_path: str) -> Tuple[bool, List[Dict]]:
    ...
    DISABLED for ncps-mlx: This is an MLX-specific project.
    """
    # Always return no issues for MLX project
    return False, []

class PrecisionReducingVisitor(ast.NodeVisitor):
    """AST visitor to find precision-reducing casts and Python operators (NOT tensor conversions for MLX)."""
    
    def __init__(self):
        self.precision_reducing_casts = []
        self.tensor_conversions = []  # Keep for compatibility but won't populate it
        self.python_operators = []
        self.numpy_imports = set()
        self.numpy_aliases = set()
        self.torch_imports = set()  # Track PyTorch imports (shouldn't be here)
        self.backend_imports = set()  # Keep for compatibility
        self.backend_usage = []
    ...
    def visit_Import(self, node):
        ...
        for name in node.names:
            if name.name == 'numpy':
                ...
                self.torch_imports.add(f"import {name.name}")
    ...
    def visit_ImportFrom(self, node):
        ...
        if node.module == 'numpy':
            ...
            for name in node.names:
                self.torch_imports.add(f"from {node.module} import {name.name}")
        self.generic_visit(node)
