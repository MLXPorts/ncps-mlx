"""Continuous-Time GRU cell implemented with MLX."""
...
from .liquid_utils import sigmoid
...
class CTGRUCell(nn.Module):
    ...
    def _ensure_parameters(self, input_dim: int) -> None:
        if self.input_linear is None or self.input_linear.weight.shape[1] != input_dim:
            self.input_linear = nn.Linear(input_dim, 3 * self.units, bias=False)
        ...
        if self.recurrent_linear is None:
            self.recurrent_linear = nn.Linear(self.units, 3 * self.units, bias=True)
            scale = mx.sqrt(2.0 / (2 * self.units))
            self.recurrent_linear.weight = scale * mx.random.normal(self.recurrent_linear.weight.shape)
            self.recurrent_linear.bias = 0.1 * mx.ones((3 * self.units,))
